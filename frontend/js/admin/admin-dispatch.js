...
function setDispatchPlannerMode(mode = "create") {
  ...
  if (heading) heading.textContent = dispatchPlannerMode === "live" ? "Dispatch Details" : "Dispatch Planner";
...
function setDispatchPlannerStep(step, options = {}) {
  dispatchPlannerStep = Math.max(1, Math.min(2, Number(step) || 1));
...
function updateDispatchPlannerActions() {
  ...
  dispatchSetElementVisible(dispatchNowButton, creating && dispatchPlannerStep === 2);
  dispatchSetElementVisible(clearButton, creating && dispatchPlannerStep === 2 && destinationCount > 0);
...
function prepareDispatchPlannerForTruck(truck) {
  ...
  if (truck.dispatch) {
    if (isNewSelection) {
      ...
      if (panel) {
        panel.innerHTML = '<div class="dispatch-panel-empty"><div><strong>Loading Dispatch Details</strong><p>The finalized route and current progress are being loaded.</p></div></div>';
      }
    }
  ...
  dispatchRoutingRequestTimer = setTimeout(async () => {
    ...
    try {
      ...
      const journey = buildDispatchPlannedJourney(startPoint, wmo, items, {
        costLookup,
        lockedPrefixCount: items.length
      });
      if (routeStops.length && !journey.plannedStops.length) {
        throw new Error("No complete optimized remaining route could be calculated.");
      }
...
function renderDispatchReadOnlyRoute(stops = [], currentStop = null) {
  ...
  return `
    <section class="dispatch-readonly-route" aria-labelledby="dispatchReadOnlyRouteHeading">
      <div class="dispatch-readonly-section-heading"><small id="dispatchReadOnlyRouteHeading">Route</small><span>Saved order</span></div>
...
function renderDispatchTicketDetails(details) {
  ...
  panel.innerHTML = `
    <div class="dispatch-readonly-heading">
      <div><small>Finalized route</small><strong>${dispatchEscape(ticket.truck_name_snapshot || ticket.truck_id)}</strong></div>
...
function buildDispatchRouteLayers(journey, routeCoordinates, startPoint, wmoPoint, options = {}) {
  ...
  L.polyline(
    ...
    .bindTooltip("Continuous road-following suggested route")
...
function renderDispatchDraftOnLiveMap(options = {}) {
  ...
  dispatchRoutingRequestTimer = setTimeout(async () => {
    ...
    try {
      ...
      activateDispatchPlannedLayerGroups(layers);
      dispatchLastSuccessfulRouteCoordinates = routeCoordinates.map(([lat, lng]) => ({ lat, lng }));
      dispatchLastSuccessfulRouteState = { journey, originSource: routeOrigin.source };
      dispatchLastRouteDistanceMeters = Number.isFinite(Number(journey.total_cost_meters))
        ? Number(journey.total_cost_meters)
        : null;
      dispatchLastRoutingSignature = signature;
...
function renderDispatchReportMap(stops = [], routeLogs = []) {
  ...
  if (suggested.length >= 2) {
    L.polyline(suggested.map((point) => [point.lat, point.lng]), {
      ...
      opacity: 0.8,
      dashArray: "9 8"
    }).bindTooltip("Suggested route between persisted dispatch waypoints")
...
function renderDispatchReportDetails(data = {}) {
  ...
  body.innerHTML = `
    ...
    <section class="dispatch-report-section"><h4>Route Map</h4><div class="dispatch-report-map-wrap"><div id="dispatchReportMap"></div></div><div class="dispatch-report-map-legend"><span><i class="actual"></i>Dark green: actual GPS trail</span><span><i class="suggested"></i>Blue: suggested route</span></div>${logs.length < 2 ? '<p class="dispatch-report-empty">No actual GPS trail recorded.</p>' : ""}</section>
